...
public class BoardViewer extends Viewer {
    ...
    public static void showBoardView(Board board, Bench bench, int limit) {
        String boardView = String.valueOf(getBoardViewHeader(board, limit)) +
                BattlefieldViewer.getCreaturesRow(board.getCreatures()) +
                getBoardViewFooter(board);
        MessageController.print(boardView);
    ...
    private static String getBenchView(Bench bench, boolean enumerate) {
        ...
        window.lineWithAngles();
        window.list(
                bench.getCreaturesWithDummys().stream().map(c -> c.getShopView(true, false, true, true)).filter(c -> !c.equals("Empty") && !c.equals("Пусто")).collect(Collectors.toList()),
    ...
    private static StringBuilder getBoardViewHeader(Board board, int limit) {
        ...
        row.append("|").append(" ".repeat(OFFSET)).append(headerLine).append(" ".repeat(Constants.MAX_NAME_LEN.value - headerLine.length() + 2))
                .append("[").append(board.getCreatures().size()).append(" / ").append(limit).append("]");
    ...
    public static String benchBoardSimpleView(List<Creature> boardCreatures, List<Creature> benchCreatures, boolean withBackOption) {
        ...
        window.emptyLine();
        window.list(benchCreatures.stream().map(c -> c.getShopView(true, true, true, true)).collect(Collectors.toList()), true, boardCreatures.size() + 1, true, false);
        window.lineWithAngles();
        return window.getView();
    }
}
